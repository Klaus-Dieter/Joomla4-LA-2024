                ...
                <# if(_.isArray(data.sp_team_carousel_item)){
                    _.each(data.sp_team_carousel_item, function(carousel_item){
						...
						if(isUrlObject || isUrlString ){
							const urlObj = profileLink?.url ? profileLink : window.getSiteUrl(profileLink, carousel_item?.new_tab);			
							const {url, menu, page, type} = urlObj;
							
							switch (type) {
								case "url":
									carousel_item.person_profile_link = url;
									break;
								case "menu":
									carousel_item.person_profile_link = menu;
									break;
								case "page":
									carousel_item.person_profile_link = "index.php/component/sppagebuilder/index.php?option=com_sppagebuilder&view=page&id=" + page;
									break;
							}
						}
                            ...
                            if( _.isArray(carousel_item.team_carousel_item)){
                                content += `<ul class="sppb-carousel-extended-team-social-icon">`;
                                    _.each(carousel_item.team_carousel_item, function(inner_item_value){
										...
										if(isUrlObject || isUrlString ){
											const urlObj = socialUrl?.url ? socialUrl : window.getSiteUrl(socialUrl, inner_item_value?.new_tab);			
											const {url, menu, page, type} = urlObj;
											
											switch (type) {
												case "url":
													inner_item_value.social_url = url;
													break;
												case "menu":
													inner_item_value.social_url = menu;
													break;
												case "page":
													inner_item_value.social_url = "index.php/component/sppagebuilder/index.php?option=com_sppagebuilder&view=page&id=" + page;
													break;
											}
										}
