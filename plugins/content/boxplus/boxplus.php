...
class plgContentBoxPlus extends JPlugin {
    ...
    */
    private function getResourceRelativePath($relpath) {
        ...
        if (!self::$debug && (!file_exists($original) || file_exists($minified) && filemtime($minified) >= filemtime($original))) {
    ...
    */
    private function addStyles($id = null) {
        ...
        $document->addStyleSheet(JURI::base(true).$this->getResourceRelativePath('/media/plg_boxplus/css/boxplusx.css'));
    ...
    private function emitStyles() {
        ...
        foreach ($this->styles as $selector => $properties) {
            if (!empty($properties)) {
                ...
                $cssrule = implode('', $cssproperties);
                $cssrules[] = "{$selector}{{$cssrule}}";
            }
        }
    ...
    */
    private function addScripts() {
        $document = JFactory::getDocument();
        $document->addScript(JURI::base(true).$this->getResourceRelativePath('/media/plg_boxplus/js/boxplusx.js'), array(), array('defer' => true));
    }
    ...
    */
    private function getLocalReplacement($match) {
        ...
        foreach (array('poster','srcset','sizes') as $attr) {
            if (isset($options[$attr])) {
                $attrs["data-{$attr}"] = $options[$attr];
            }
        }
